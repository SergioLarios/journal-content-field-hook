// All modifications to the original JSP will begin and end with comments like these:
...
AUI().use('node', function (A) { A.on("domready", function(){

	var groupId = themeDisplay.getScopeGroupId();
	var selAll = 'div[data-fieldname^="Web_Content"]';
	var selInput = 'input.field';
	var addBtnSel = 'a.lfr-ddm-repeatable-add-button';
	var readonly = 'readonly';
	var selWC = 'select-web-content';
	var strucutreSel = '#_15_structureId';
	var clearWC = 'clear-web-content';
	var baseUri = '/c/portal/custom-search-wc';
	var scUri = baseUri + '?groupId=' + groupId + '&locale=' + themeDisplay.getLanguageId();
	var btnSelC = 'button.select-web-content';
	var btnClear = 'button.clear-web-content';
	...
	var windowId = Liferay.Util.getPortletNamespace('56') + 'selectWebContentWindow';
	var btnsTmpl = 
		'<div class="btn-group btns-wc">' +
		'	<button type="button" class="btn yui3-widget btn-content select-web-content">' +
		'		<i class="icon-edit"></i> ' + Liferay.Language.get('wcs.select') +
		'	</button>' +
		'	<button type="button" class="btn yui3-widget btn-content clear-web-content">' +
		'		<i class="icon-ban-circle"></i> ' + Liferay.Language.get('wcs.clear') +
		'	</button>' +
		'</div>';
	var upBtn = '<button type="button" class="btn yui3-widget btn-wc-sort btn-wc-sort-up">'
		+ '<i class=" icon-chevron-up"></i> </button>';
	var downBtn = '<button type="button" class="btn yui3-widget btn-wc-sort btn-wc-sort-down">'
		+ '<i class=" icon-chevron-down"></i> </button>';
	var pWindow =  (window.parent === window) ? window : window.parent;
	var dynEls;
	
	window['processAllWebContentField']= function () {
		
		getAllFieldNodes(function(fieldNodes) {
		
			A.Array.each(fieldNodes, function (node) {
				
				node = A.one(node);
				
				var processed = A.DataType.Boolean.parse(node.getData('processed'));
				
				if (!processed) {
					
					node.setAttribute('data-processed', 'true');
					var inp = node.one(selInput);
					var btns = A.Node.create(btnsTmpl);

					inp.hide();
					setWcName(inp);
					btns.appendTo(inp.get('parentNode'));
					btns.setStyle('margin-top', '-7px');
					btns.setStyle('margin-left', '3px');
					btns.one(btnSelC).on('click', fnSelContent);
					btns.one(btnClear).on('click', fnClearContent);
					
				}
				
			});
			
			checkSortableElements();
		
		});
		
	}
	
	window['checkAddBtnWebContentField'] = function() {

		A.all(addBtnSel).on('click', function (ev) {

			var node = ev.target;
			
			var processed = A.DataType.Boolean.parse(node.getData('ev-processed'));
			
			if (!processed) {
				
				node.setAttribute('data-ev-processed', 'true');
				processAllWebContentField();
				checkAddBtnWebContentField();
	...
	var checkSortableElements = function() {
		
		getDynEls(function(dynEls) {
			
			A.Array.each(dynEls, function (el) {
				
				if (el.repeatable) {
					
					var nodes = A.all('div[data-fieldname="' + el.name + '"]');
					var i = 0;
					var size = nodes.size() - 1;
					
					nodes.each(function(node) {
						
						var btnGroup = node.one('.btns-wc');
						var currBtns = btnGroup.all('.btn-wc-sort');
						if (currBtns) { currBtns.remove(); }
						
						if (i > 0) {
							var upNode = A.Node.create(upBtn);
							upNode.on('click', function(e){ sortEl(true, e) });
							btnGroup.append(upNode);
						}
						
						if (i < size) {
							var downNode = A.Node.create(downBtn);
							downNode.on('click', function(e){ sortEl(false, e) });
							btnGroup.append(downNode);
						}
	...
	function sortEl(isUp, e) {
		
		var node = e.target.get('parentNode').get('parentNode');
		var inp = node.one('input.field[type="text"]');
		var targetNode = isUp ? node.previous() : node.next();
		var targetInp = targetNode.one('input.field[type="text"]');
		
		var tmpVal = inp.get('value');
		inp.set('value', targetInp.get('value'));
		targetInp.set('value', tmpVal);
	...
	function getAllFieldNodes(callBack) {
		
		if (themeDisplay.isSignedIn()) {
			
			getDynEls(function(dynEls) {
				
				callBack(searchNodes(dynEls));
	...
	function getDynEls(callBack) {
		
		if (dynEls && dynEls.length > 0) {
			callBack(dynEls);
		}
		else {
			
			fetchCNID(function(classNameId) {

				try {
					
					var strucutreInp = A.one(strucutreSel);
					
					if (strucutreInp) {
						
						var strKey = strucutreInp.get('value');
						...
						Liferay.Service('/ddmstructure/fetch-structure',
							params,
							function(obj) {
								dynEls = getFieldNames(obj.xsd, 'web-content');
								callBack(dynEls);
							}
						);
	...
	function fetchCNID(callBack) {
		
		try {
			Liferay.Service('/classname/fetch-class-name-id',
	...
	function getFieldNames(xml, name) {
		...
		var dyns = xmlDoc.getElementsByTagName('dynamic-element');

		for (var i = 0; i < dyns.length; i++) {
			if (name === dyns[i].getAttribute('dataType')) {
				dynEls.push({
					dataType: dyns[i].getAttribute('dataType'),
					name: dyns[i].getAttribute('name'),
					repeatable: ("true" === dyns[i].getAttribute('repeatable'))
				});
			}
		}
	...
	function searchNodes(dynEls) {
		var nameNodes = [];
		A.Array.each(dynEls, function(dyn) {
			
			Array.prototype.push.apply(nameNodes, 
					A.all('div[data-fieldname="' + dyn.name + '"]')._nodes);
			
		});
		return nameNodes;
	}
	
	function fnSelContent(node) {

		var grandpa = node.target.get('parentNode').get('parentNode');
		var wcJson = grandpa.one(selInput).get('value');
		
		try {
			if (wcJson) {
				
				var jsonObject = JSON.parse(wcJson);
				
				if (jsonObject.defaultStructure) {
					openWindow(jsonObject.defaultStructure);
				}
				else {
					openWindow();
				}
		...
		Liferay.provide(pWindow, fnSelWC,
			function (groupId, contentId, title, urlTitle) {
				selectWebContentInp(groupId, contentId, title, urlTitle, grandpa.one(selInput));
			}
		);

	}
	
	function fnClearContent(node) {

		var grandpa = node.target.get('parentNode').get('parentNode');
		grandpa.one(selInput).set('value', '');
		setWcName(grandpa.one(selInput));

	}

	function openWindow(defaultStructure) {
		
		var url = scUri;
		
		if (defaultStructure) {
			url = url + '&structure=' + defaultStructure.key + '&hideStructure=true';
		}
		...
		Liferay.provide(pWindow, fnCloseWindow,
			function () {
				pWindow.Liferay.Util.Window.getById(windowId).destroy();
			},
			['liferay-util-window']
		);
	...
	function selectWebContentInp(groupId, contentId, title, urlTitle, selInput) {
		
		var befObj = selInput.get('value');
		var newobj = {}
		
		if (befObj) {
			try {
				newobj = JSON.parse(befObj);
			} catch (e) {}
		}
		
		newobj.groupId = groupId;
		newobj.contentId = contentId;
		newobj.title = title;
		newobj.urlTitle = urlTitle;
		newobj.hasBeenImported = false;
		newobj.targetContentAvailable = false;
		
		selInput.set('value', JSON.stringify(newobj));
		setWcName(selInput);
	}
	
	function setWcName(inp) {
		
		var contentNotFond = inp.get('parentNode').one('.wc-not-found');
		if (contentNotFond) { contentNotFond.remove(); }
		var contentSelected = inp.get('parentNode').one('.wc-selected');
		if (contentSelected) { contentSelected.remove(); }
		
		var obj = null;
		try { obj = JSON.parse(inp.get('value')); } catch (e) {}
		
		if (obj && obj.groupId && obj.contentId && obj.title && obj.urlTitle && 
				( !obj.hasBeenImported || (obj.hasBeenImported && obj.targetContentAvailable)) ) {
			
			var portletURL = Liferay.PortletURL.createRenderURL();
			portletURL.setPlid(themeDisplay.getPlid());
			portletURL.setPortletId('15');
			portletURL.setParameter('struts_action', '/journal/edit_article');
			portletURL.setParameter('groupId', obj.groupId);
			portletURL.setParameter('articleId', obj.contentId);
			portletURL.setParameter('status', -1);

			var cnt = '<a target="_blank" href="' + portletURL.toString() + 
				'&doAsGroupId=' + obj.groupId + '" class="wc-selected">' + obj.title + '</a>';
			
			inp.insert(A.Node.create(cnt), 'after');
			
		}
		else {
			
			var cnt = '<span class="wc-not-found"><strong>' + 
						Liferay.Language.get('wcs.web-content-not-found') + 
					  '</strong></span>';
